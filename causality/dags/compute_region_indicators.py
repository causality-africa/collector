...
avg_indicators = ["cpi"]
sum_indicators = ["births"]

derived_indicators = {
    # "gdp_per_capita": {
    #     "dependencies": ["gdp", "population"],
    #     "calculate": lambda values: (
    #         values["gdp"] / values["population"] if values["population"] else None
    #     ),
    # },
}
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            ...
            for indicator_code in all_indicators:
                ...
                if not years:
                    logging.warning(
                        f"No data years found for indicator {indicator_code}"
                    )
                    continue
                ...
                calculation_method = (
                    calculate_average
                    if indicator_code in avg_indicators
                    else calculate_sum
                )
                ...
                # Commit after each indicator to manage memory
                if results:
                    ...
                    results = []  # Reset results after commit


def calculate_derived_indicators(**kwargs):
    """Calculate derived indicators for regions based on their specific available years."""
    results = []
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            # Get all regions
            ...
            for derived_code, config in derived_indicators.items():
                ...
                for dep_code in config["dependencies"]:
                    ...
                    if not dep_id:
                        logging.warning(
                            f"Dependency indicator code {dep_code} not found"
                        )
                ...
                # Commit after each derived indicator
                if results:
                    insert_data_points(results, cur)
                    conn.commit()
                    logging.info(
...
with DAG(
    ...
    description="Calculate region indicators from location data",
